Public Module _Shared
    ...
    Public Function GetResourceImage(imageData As Byte()) As Drawing.Image
        Dim Result As Drawing.Image

        Using stream As New IO.MemoryStream(imageData)
            Result = Drawing.Image.FromStream(stream)
        End Using

        Return Result
    End Function

    Public Function GetScreenWorkingArea() As Drawing.Rectangle
    ...
    Public Sub Initialise(errorLogLocation As String, forceFormBackColor As Boolean, loadSettingsCache As Boolean, performanceCounterLocation As String, useInvariantCulture As Boolean)
        Try
            Settings.Icon = GetResourceIcon(My.Resources.INFO)
