Namespace CommonRoutines.Extensions
    ...
    Public Module System_Windows_Forms_ComboBox

        <Runtime.CompilerServices.Extension()> Public Function GetSelectedComboItemValue(c As ComboBox) As String
        ...
        <Runtime.CompilerServices.Extension()> Private Sub PopulateComboItems(c As ComboBox, items As IEnumerable(Of Models.ComboItem))
        ...
        <Runtime.CompilerServices.Extension()> Public Sub PopulateComboItems(c As ComboBox, items As IEnumerable(Of String))
            c.PopulateComboItems(Nothing, items)
        End Sub

        <Runtime.CompilerServices.Extension()> Public Sub PopulateComboItems(c As ComboBox, fixedItems As IEnumerable(Of String), items As IEnumerable(Of String))
        ...
        <Runtime.CompilerServices.Extension()> Public Sub PopulateComboItems(c As ComboBox, items As IDictionary(Of String, String))
            c.PopulateComboItems(Nothing, items)
        End Sub

        <Runtime.CompilerServices.Extension()> Public Sub PopulateComboItems(c As ComboBox, fixedItems As IDictionary(Of String, String), items As IDictionary(Of String, String))
            Dim ComboItems As New List(Of Models.ComboItem)
            If fixedItems IsNot Nothing AndAlso fixedItems.Count > 0 Then
        ...
        <Runtime.CompilerServices.Extension()> Public Sub SelectComboItem(c As ComboBox, item As String)
            c.SelectComboItem(item, True)
        End Sub

        <Runtime.CompilerServices.Extension()> Public Sub SelectComboItem(c As ComboBox, item As String, autoAdd As Boolean)
            c.SelectedItem = Nothing
